...
def environment(env_type, env_name, env_family, agent_ids, kwargs):
    if env_type == "pz":
        ...
        env = LBFWrapper(
            map_name=env_name, reward_aggr=None, agent_ids=agent_ids, **kwargs
        )
            ...
            if args.use_tdlamda:
                for ep_idx in range(return_lambda.size(0)):
                    ...
                    if is_truncated[ep_idx]:
                        last_obs_ep_idx = torch.from_numpy(last_obs[ep_idx]).float()
                        last_state_ep_idx = torch.from_numpy(last_state[ep_idx]).float()
                        last_avail_action_ep_idx = torch.tensor(
                            last_avail_action[ep_idx], dtype=torch.bool
                        )
                        last_actions_ep_idx = actor.act(
                            last_obs_ep_idx,
                            eps=epsilon,
                            avail_action=last_avail_action_ep_idx,
                        )
                        last_return_lambda = target_critic(
                            state=last_state_ep_idx,
                            observations=last_obs_ep_idx,
                            actions=last_actions_ep_idx,
                            avail_actions=last_avail_action_ep_idx,
                        )
                        last_return_lambda = torch.gather(
                            last_return_lambda,
                            dim=-1,
                            index=last_actions_ep_idx.unsqueeze(-1),
                        ).squeeze()
                    else:
                        last_return_lambda = 0
                    for t in reversed(range(ep_len)):
                        if t == (ep_len - 1):
                            next_action_value = last_return_lambda
                        else:
