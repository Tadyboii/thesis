...
class Args:
    ...
    """If True, render the evaluation environment during evaluation (human)"""
    use_wnb: bool = False
...
class Actor(nn.Module):
    ...
    def act(self, x, actions=None):
        ...
        if actions is None:
            ...
            # actions = (torch.tanh(u) + 1) / 2
        else:
            u = torch.atanh(torch.clamp(actions, -0.999999, 0.999999))
            # a = actions * 2 - 1                     # [0,1] → [-1,1]
            # u = torch.atanh(torch.clamp(a, -0.999999, 0.999999))
        ...
        log_prob_tanh = torch.sum(
            torch.log(1.0 - a.pow(2) + 1e-6), dim=-1
        )
        # log_probs = log_prob_u - log_prob_tanh - np.log(2)
        log_probs = log_prob_u - log_prob_tanh
...
def environment(env_type, env_name, env_family, agent_ids, kwargs):
    if env_type == "pz":
        if env_family == "custom":
            env = SwarmCagingEnv(**kwargs)
            return PettingZooWrapper(env=env, agent_ids=agent_ids)
        else:
        ...
        raise NotImplementedError(f"Unknown env_type {env_type}")
...
if __name__ == "__main__":
    ...
    # kwargs = {"render_mode": None}  # {"render_mode":'human',"shared_reward":False}
    # eval_kwargs = {"render_mode": "human"} if args.render_eval else {"render_mode": None}
    kwargs = {
        # "render_mode": None,
    ...
    if args.checkpoint_path and os.path.isfile(args.checkpoint_path):
        ...
        # print(f"Loaded checkpoint from {args.checkpoint_path} at step {step}"
        checkpoint = torch.load(args.checkpoint_path, map_location=device)
        actor.load_state_dict(checkpoint["actor_state_dict"])
        # Not loading: critic, optimizers, or step counters
        print(f"Loaded actor from {args.checkpoint_path}")
    ...
    while step < args.total_timesteps:
        ...
        if training_step % args.eval_steps == 0:
            ...
            # Save best model checkpoint when evaluation improves
            try:
                mean_eval = float(np.mean(eval_ep_reward))
            except Exception:
                mean_eval = None
            if mean_eval is not None and mean_eval > best_eval_reward:
                ...
                writer.add_text(
                    "checkpoint",
                    f"Saved best checkpoint at step={step} mean_eval={best_eval_reward}",
                )
