...
class PettingZooWrapper(CommonInterface):
    # Inspired from :https://github.com/uoe-agents/epymarl/blob/main/src/envs/pz_wrapper.py
    def __init__(self, family=None, env_name=None, agent_ids=False, env=None, vector_rewards=False, **kwargs):
        """
        PettingZoo has families of environments (Atari, Butterfly, Classic, MPE, SISL)
        if order to use the pursuit game (sisl family), you usually import it like this:
         >>>>from pettingzoo.sisl import pursuit_v4
        ...
        if env is not None:
            ...
            module = importlib.import_module(f"pettingzoo.{family}.{env_name}")
            self.env = module.parallel_env(**kwargs)
        self.env.reset()
        self.n_agents = self.env.num_agents
        self.agents = self.env.agents
        ...
        self.agent_ids = agent_ids
        # If True, `step()` will return a per-agent reward vector. Otherwise returns scalar summed reward.
        self.vector_rewards = vector_rewards
    ...
    def step(self, actions):
        ...
        if done:
            ...
            self.last_obs = obs
        # Return per-agent reward vector if requested, otherwise return scalar summed reward
        if self.vector_rewards:
            return obs, np.array(rewards, dtype=np.float32), done, truncated, info
        else:
            return obs, float(np.sum(rewards)), done, truncated, info
